...
class HistoryController extends Controller
{
	...
	public function actionIndex() 
	{
		...
		$setting = ReportHistory::model()->findByPk(1,array(
		...
		$dataProvider = new CActiveDataProvider('ReportHistory', array(
			'criteria'=>$criteria,
			'pagination'=>array(
				'pageSize'=>10,
			),
	...
	public function actionAdd() 
	{
		...
		if(isset($_POST['ReportHistory'])) {
				...
				if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
					if($model->save()) {
						echo CJSON::encode(array(
							...
							'msg' => '<div class="errorSummary success"><strong>'.Yii::t('phrase', 'ReportHistory success created.').'</strong></div>',
						));
					} else {
			...
			*/
			...
			if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
				if($model->save()) {
					Yii::app()->user->setFlash('success', Yii::t('phrase', 'ReportHistory success created.'));
		...
		$this->pageTitle = Yii::t('phrase', 'Create Report Histories');
		$this->pageDescription = '';
		$this->pageMeta = '';
	...
	public function actionEdit($id) 
	{
		...
		if(isset($_POST['ReportHistory'])) {
				...
				if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
					if($model->save()) {
						echo CJSON::encode(array(
							...
							'get' => Yii::app()->controller->createUrl('manage'),
							'id' => 'partial-report-history',
							'msg' => '<div class="errorSummary success"><strong>'.Yii::t('phrase', 'ReportHistory success updated.').'</strong></div>',
						));
					} else {
						print_r($model->getErrors());
					}
			...
			*/
			...
			if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
				if($model->save()) {
					Yii::app()->user->setFlash('success', Yii::t('phrase', 'ReportHistory success updated.'));
					//$this->redirect(array('view','id'=>$model->id));
					$this->redirect(array('manage'));
				}
			}
		...
		$this->pageTitle = Yii::t('phrase', 'Update Report Histories');
		$this->pageDescription = '';
		$this->pageMeta = '';
	...
	public function actionRunAction() {
		...
		if(count($id) > 0) {
			...
			if($actions == 'publish') {
				ReportHistory::model()->updateAll(array(
					'publish' => 1,
				),$criteria);
				...
				ReportHistory::model()->deleteAll($criteria);
			}
		}
